...
def load_schematic(filename):
    ...
    width = nbt_data['Width'].to_python()
    height = nbt_data['Height'].to_python()
    length = nbt_data['Length'].to_python()
    
    # Read block data (assuming 'Blocks' tag holds block ids)
    blocks = []
    block_data = nbt_data['Blocks'].to_python()  # This is an array of block IDs
